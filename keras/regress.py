...
def generate_data(mus, sigs, norms, max_size):
  ...
  mus = numpy.broadcast_to(mus, (max_size, 1, batches)).T
  sigs = numpy.broadcast_to(sigs, (max_size, 1, batches)).T
...
testsig_mu = avg(sig_mu_range) * numpy.ones(ntests)
testsig_sigma = avg(sig_sigma_range) * numpy.ones(ntests)

testbkg_mu = avg(bkg_mu_range) * numpy.ones(ntests)
testbkg_sigma = avg(bkg_sigma_range) * numpy.ones(ntests)
...
testbkginputs = generate_data(testbkgmus, testbkgsigmas, testbkgnorms, max_size)
for i in (range(n_bkgs-1)):
  ...
  testbkginputs = testbkginputs.cat(generate_data(testbkgmus, testbkgsigmas, testbkgnorms, max_size))
